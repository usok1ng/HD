...
struct MeasureListView: View {
    ...
    // MeasureState를 환경으로 뿌려 모드가 한 군데서만 관리되게 하여 화면들 동기화를 쉽게 하려는 의도
    @State public var mode = 0
    @EnvironmentObject var measureState: MeasureState
    // 0 real
    // 1 case 1
    // 2 case 2
    // 3 case 3
    ...
    var body: some View {
        ...
        if measureState.showModeLabel {
            Text("Refreshed!\(measureState.mode)")
                .font(.system(size: 9))
                .padding(8)
...
//                    mode += 1
//                    mode = mode % 4
//                    measureState.mode = (measureState.mode + 1) % 4
                    measureState.showModeLabel = true
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                                measureState.showModeLabel = false
                    }
                    points.removeAll()
                    await measure.LoadRestApiAsync(isWait: false)
                    setPoint(done_idx: "")
